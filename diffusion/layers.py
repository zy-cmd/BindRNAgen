...
class MultiHeadCrossAttention(nn.Module):
    def __init__(self, dim_q: int, dim_kv: int, hidden_dim: int = 256, num_heads: int = 8) -> None:
        super().__init__()
        
        assert hidden_dim % num_heads == 0, "hidden_dim must be divisible by num_heads"

        self.num_heads = num_heads
        self.dim_head = hidden_dim // num_heads # 计算每个头的维度
        
        self.to_q = nn.Linear(dim_q, hidden_dim, bias=False)
        self.to_k = nn.Linear(dim_kv, hidden_dim, bias=False)
        self.to_v = nn.Linear(dim_kv, hidden_dim, bias=False)
        
        self.to_out = nn.Linear(hidden_dim, dim_q)

    def forward(self, x: torch.Tensor, context: torch.Tensor):
        b, c, h, w = x.shape
        
        x_reshaped = x.view(b, c, h * w).permute(0, 2, 1)  # -> [B, H*W, C]
        context_reshaped = context.unsqueeze(1) # -> [B, 1, C_kv]
        
        q = self.to_q(x_reshaped)      # -> [B, H*W, hidden_dim]
        k = self.to_k(context_reshaped)  # -> [B, 1, hidden_dim]
        v = self.to_v(context_reshaped)  # -> [B, 1, hidden_dim]


        q = rearrange(q, 'b n (h d) -> b h n d', h=self.num_heads) # -> [B, num_heads, H*W, dim_head]
        k = rearrange(k, 'b n (h d) -> b h n d', h=self.num_heads) # -> [B, num_heads, 1,   dim_head]
        v = rearrange(v, 'b n (h d) -> b h n d', h=self.num_heads) # -> [B, num_heads, 1,   dim_head]

        scale = self.dim_head ** -0.5
        scores = torch.einsum('b h i d, b h j d -> b h i j', q, k) * scale # -> [B, h, H*W, 1]
        
        attn = torch.softmax(scores, dim=-1) # -> [B, h, H*W, 1]

        out = torch.einsum('b h i j, b h j d -> b h i d', attn, v) # -> [B, h, H*W, dim_head]
        

        out = rearrange(out, 'b h n d -> b n (h d)') # -> [B, H*W, hidden_dim]
