...
from utils.utils import default, extract, linear_beta_schedule
...
class Diffusion(nn.Module):
    ...
    def p_sample_loop(self, classes, image_size, cond_weight, get_cross_map=False):
        ...
        if get_cross_map:
            ...
            for i in reversed(range(0, self.timesteps)):
                img = sampling_fn(x=img, t=torch.full((b,), i, device=device, dtype=torch.long), t_index=i)
                imgs.append(img.cpu().numpy())
    ...
    def p_sample(self, x, t, t_index):
        ...
        model_mean = sqrt_recip_alphas_t * (x - betas_t * self.model(x, time=t,classes = None) / sqrt_one_minus_alphas_cumprod_t)
...
# )
# sample_bs = 16
# output = diffusion.sample_cross(classes=classes,
#                                 shape=(sample_bs, 4, 16, 16),#image size
#                                 cond_weight= 0 )
# print(output)
# # output = diffusion(x, classes)
# # print(output)
