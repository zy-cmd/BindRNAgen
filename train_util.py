...
class TrainLoop:
    def __init__(
        ...
        # Dataloader
        seq_dataset = SequenceDataset(self.encode_data,self.protein_name)
    ...
    def save_model(self, epoch):
        ...
        torch.save(
            checkpoint_dict,
            f"checkpoints_train_part/epoch_{epoch}_{self.model_name}.pt",
        )
...
class TrainLoopLucaOne:
    def __init__(
        ...
        # Dataloader
        seq_dataset = SequenceLucaDataset(self.encode_data,self.protein_name)
        self.train_dl = DataLoader(seq_dataset, batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=True)
    ...
    def save_model(self, epoch):
        ...
        torch.save(
            checkpoint_dict,
            f"checkpoints_train_lucaone_v2/epoch_{epoch}_{self.model_name}.pt",
        )
...
class TrainLoopStable:
    def __init__(
        ...
        self.vae = vae_model
        # Freeze the VAE parameters - we are not training it
